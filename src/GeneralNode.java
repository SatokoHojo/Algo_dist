...
public class GeneralNode extends Node {
    static int nb_nodes; // assuming for now we calculate it somehow before starting
    static int delta; // same
    ...
    public void onStart() {
        ...
        */
        ...
        for (Node n : nei) {
            if (n.getID() < getID()) {
                fathers.set(currently_free, n);
                currently_free++;
            }
        }
        for (int i = currently_free; i < delta; i++) {
            fathers.set(i, this);
